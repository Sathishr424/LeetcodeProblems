# Last updated: 6/11/2025, 1:24:31 am
class Solution:
    def findXSum(self, nums: List[int], k: int, x: int) -> List[int]:
        ...
        def isValid(num, cnt, index, left_limit):
            if freq[num] != cnt: return False
        ...
        def go(i):
            ...
            while left and not isValid(left[0][1], left[0][0], left[0][2], i - k):
                heapq.heappop(left)
                extra -= 1
            ...
            while right and \
                (
                    len(left) - extra < x or ( 
                        -right[0][0] > left[0][0] or ( 
                            -right[0][0] == left[0][0] and -right[0][1] > left[0][1] 
                        ) 
                    ) 
                ):
                if not isValid(-right[0][1], -right[0][0], right[0][2], i - k):
                    heapq.heappop(right)
                    continue
            ...
            while len(left) - extra > x:
                ...
                if not isValid(num, c, index, i - k):
                    extra -= 1
                else:
